# Integration Regression Eval: Scope Request -> Design Changes -> Implement Changes

この文書は、mizchi 側の外部 Skill `empirical-prompt-tuning` の手順を、このリポジトリの `scope-request`, `design-changes`, `implement-changes` に適用するための統合回帰 eval メモです。

日常の改善は各 Skill の `evals/` で行い、この文書は複数 Skill をつないだときの受け渡し崩れや統合時の退行を確認する用途に使います。

対象 Skill:

- `skills/scope-request/SKILL.md`
- `skills/design-changes/SKILL.md`
- `skills/implement-changes/SKILL.md`

## Iter 0: Static Review

- `scope-request`: 目的、完了条件、制約はあるが、`design-changes` に渡す必須情報の最低セットが明文化されていない。
- `design-changes`: 変更対象、リスク、テスト戦略はあるが、`implement-changes` に渡す実装単位と停止条件の粒度がやや曖昧。
- `implement-changes`: TDD 方針は明確だが、テストを実行できない場合の扱い、Red の確認結果の表現、検証への引き継ぎ粒度が薄い。
...
1. [critical] `scope-request` と `design-changes` の出力を分けたまま、`implement-changes` に入る前の完了条件と対象範囲を明示する
2. `design-changes` で変更対象と変更対象外を分離する
3. `implement-changes` で最初の 1 テストと Red -> Green -> Refactor の現在地を示す
4. 未解決事項または承認待ち事項があれば、実装前に止める条件として明示する

### Scenario B: Edge

依頼は機能追加だが、外部依存追加の可能性と仕様未確定の部分がある。既存テストは不十分で、どのコマンドを使うかも完全には分からない。executor には、無理に実装へ進まず、停止条件と追加確認点を含む形で `scope-request`、`design-changes`、`implement-changes` の出力を作らせる。

Requirements checklist:

1. [critical] 依存追加や仕様未確定を `Ask first` の停止条件として扱う
2. `scope-request` で未解決事項と前提を分離する
3. `design-changes` でリスクと回避策を対にして書く
4. `implement-changes` でテスト実行不能または不明な前提を隠さず、次の一手を確認行為に寄せる
...
- Scenario B: [critical] drop なし
  - Issue: `implement-changes` は本来実装用スキルだが、今回はコード変更禁止かつ停止条件付き deliverable が要求され、停止時の表現が曖昧だった
  - Cause: `implement-changes` が Red 開始前提で書かれており、着手条件未充足時の phase と出力項目が定義されていなかった
  - General Fix Rule: 実装系スキルには、着手条件未充足時に `Blocked` として止まる分岐と、そのときの出力項目を明示する
- Scenario A: [critical] drop なし
  - Issue: JSON 追加のような曖昧な依頼では、対象範囲や最小仕様が入力不足のまま `implement-changes` に渡る
  - Cause: 上流で未確定要件は拾えているが、`implement-changes` に入れる条件と入れない条件が Skill 間で固定されていなかった
  - General Fix Rule: `design-changes` から `implement-changes` へ、着手条件、対象範囲、停止条件を固定項目として受け渡す
...
- Re-seen: `missing handoff contract between adjacent skills` - `design-changes` から `implement-changes` へ渡す固定項目が不足していた
- Added: `blocked phase missing before Red` - Red 開始前提が欠ける場面で `implement-changes` の phase 表現が曖昧になる

### Next fix proposal

- `design-changes` に `implement-changes` へ渡す着手条件 / 対象範囲 / 停止条件を足し、`implement-changes` に `Blocked` phase と未着手時の出力分岐を追加する
...
- `design-changes` に `implement-changes` に渡す着手条件、対象範囲、停止条件を追加
- `implement-changes` に着手条件確認ステップ、`Blocked` phase、未着手時の停止出力を追加
...
- Scenario A: [critical] drop なし
  ...
  - General Fix Rule: 曖昧な機能追加依頼では、実装前に「対象範囲」「成功系の最小仕様」「非互換にしない条件」を確認し、未確定なら `implement-changes` を `Blocked` で止める
- Scenario B: [critical] drop なし
...
- `scope-request` に「機能追加依頼で実装前に最低限確認すべき仕様粒度」の例を追加するかを hold-out scenario 後に判断する
...
- Hold-out: [critical] drop なし
  - Issue: 実コードや既存 CLI 構造を見ずに、`implement-changes` の具体的な変更ファイル名までは確定できなかった
...
- Re-seen: `missing handoff contract between adjacent skills` - 再発なし。`design-changes` から `implement-changes` への handoff は hold-out でも維持された
- Re-seen: `blocked phase missing before Red` - 再発なし。承認待ちケースでも `Blocked` と `Red` を混同しなかった

### Next fix proposal

- 現時点では Skill 本文の追加修正は不要。次に着手するなら `scope-request` の例示追加ではなく、別 flow の hold-out を増やす
...
- **Pattern name**: missing handoff contract between adjacent skills
  - Example: `design-changes` の出力は出たが、`implement-changes` が何を固定情報として受け取る前提か分からない
...
1. `scope-request` -> `design-changes` -> `implement-changes` の受け渡し必須項目を明文化する
2. `implement-changes` の Red 確認結果とテスト不能時の扱いを補強する
