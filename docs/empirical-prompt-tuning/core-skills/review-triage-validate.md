# Integration Regression Eval: Review Changes -> Triage Review Feedback -> Validate Fix

この文書は、mizchi 側の外部 Skill `empirical-prompt-tuning` の手順を、このリポジトリの `review-changes`, `triage-review-feedback`, `validate-fix` に適用するための統合回帰 eval メモです。
...
- `skills/triage-review-feedback/SKILL.md`
- `skills/validate-fix/SKILL.md`

## Iter 0: Static Review

- `review-changes`: レビュー観点は厚いが、`triage-review-feedback` に渡すべき最小情報が明文化されていない。
- `triage-review-feedback`: 採用、保留、却下の判断はあるが、優先順位と実装可能な対応方針の粒度が揃っていない。
...
1. [critical] `review-changes` の Must-fix / Should-fix / Nice-to-have を `triage-review-feedback` で採用、保留、却下へ変換し、その理由を残す
2. `review-changes` の各指摘に根拠がある
3. `triage-review-feedback` で採用した指摘に優先順位または対応方針がある
...
1. [critical] 仕様判断が必要な指摘は `triage-review-feedback` で保留または追加確認に回し、自動採用しない
2. `review-changes` が仕様不一致や安全性の観点を優先して扱う
...
- Scenario B: [critical] drop なし
  ...
  - Cause: `triage-review-feedback` の境界が効いており、仕様論点は保留、安全性論点は採用として切り分けられた
...
1. `review-changes` -> `triage-review-feedback` の受け渡し項目と判断写像を明文化する
2. `triage-review-feedback` の採用時フォーマットに優先順位と対応方針を固定項目として入れる
