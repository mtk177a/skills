...
- 実行主体や内部 orchestration より、利用場面を優先して書く
...
- trigger 判断に不要な内部実装の説明を frontmatter に詰め込む書き方
...
このリポジトリでは分類ディレクトリを増やさず、Skill 名または `description` で差を表現します。たとえば agent 固有であることが重要なら、名前や説明にその前提を含めます。
...
- 単体改善を優先し、まずは 1 Skill ごとの Iter 0、scenario、requirements checklist を整える
...
- Skill 本文では、別 agent / subagent の利用を既定動作にしない
- 別 agent / subagent は、ユーザーが明示した場合や高リスク・高不確実性の補強として提案するに留める
