...
- 行コメント本文には `must` / `should` / `suggestion` / `question` / `nit` / `note` のラベルを付け、レビュー要約は簡潔な全体判断にする。
...
7. 行コメント本文の先頭に正規ラベルを付ける。
8. コメントを `事実 → 影響 → 期待` の順で組み立てる。
9. 必須対応か任意対応か、確認だけでよいかを曖昧にしない。
10. 必要以上に解法を決め打ちせず、レビュイーがよりよい解法を選べる余地を残す。
11. 人ではなくコード、挙動、差分を主語にする。
12. `この条件` のような抽象語を避け、必要な識別子、条件式、値遷移を明示する。
13. トーン未指定時は `柔らかめ` を既定とし、まず観測した事実や懸念、次に短い理由や影響、最後に期待する修正や確認を書く。
14. `柔らかめ` では詰問調を避け、意図が推測できる場合は短く受け止めたうえで、懸念と具体案や確認へつなぐ。
15. ラベルの強さを、語尾や依頼表現でも調整する。
16. 根拠の強さに応じて、断定、懸念表現、確認質問を使い分ける。
17. 好みレベルの話は差し止め理由扱いせず、提案として書く。
18. 再レビュー時は `Resolved` / `Remaining` / `New` を先に整理する。
19. 積み重ね PR では、この PR で直すか後続 PR で許容するかを文面に含める。
20. `must` の指摘は、何が直れば Close できるかが分かる粒度で書く。
21. レビュー要約や Approve 時の補足コメントは、詳細なラベル別一覧ではなく、短い全体判断として返す。
...
- レビュー要約では、正規ラベルごとの詳細な整理を避け、全体判断と主要アクションだけを短く書く
