---
name: break-failure-loop
description: エージェントが同じエラーや方針で2回以上失敗したときに、続行せず、仮説・確認済み事項・未確認事項・次の分岐を整理したいときに使う。
license: MIT
---

> **注記:** 英語版 (`SKILL.md`) が正本です。このファイルは参考訳であり、内容に差異がある場合は英語版を優先してください。

# Break Failure Loop

## 目的

- 同じ失敗の反復を止め、変更量だけが増える状態を防ぐ。
- 仮説更新のない試行錯誤を止め、次の一手を 1 つに絞る。

## 使う条件

- 同じテストが 2 回以上失敗している
- 同じ修正方針で解決していない
- エラーの原因仮説が更新されていない
- 変更量だけが増えている
- 構造的に異なる分岐を検証せず、同じ設計近傍の変更が続いている

## やること

- これまでの試行を時系列で整理する
- 成功した確認と失敗した確認を分ける
- 現在の仮説を最大 3 つに絞る
- 追加で読むべきファイルを最大 5 つに絞る
- 同じ設計前提に停滞していないか確認する
- 次の一手を 1 つだけ提案する
- 同じ設計近傍に停滞している場合は、次の一手を構造的に異なる分岐か探索拡張への引き継ぎにする
- 実装継続が危険な場合は停止を提案する

## 手順

1. 直近の試行を古い順に並べる。
2. 何を変えたか、何を確認したか、結果がどうだったかを分ける。
3. 確認できた事実と、まだ推測のままのことを分ける。
4. 現在の仮説を最大 3 つに絞り、各仮説に根拠を付ける。
5. 追加で読むべきファイルを最大 5 つに絞る。
6. 直近の試行が prompt の言い回し、正規表現、出力正規化、リトライ、局所パッチだけで同じ設計前提の周辺に留まっていないか確認する。
7. 探索の多様化が必要で、関連 Skill が使えない場合は、構造的に異なる分岐を 1 つ直接提案する。
8. 次に確認することを 1 つだけ決める。
9. 同じ仮説や設計アンカーのまま続けるだけなら、続行ではなく停止を提案する。

## 出力フォーマット

- 現在の状況: ...
- 試したこと:
  - ...
- 分かったこと:
  - ...
- まだ分かっていないこと:
  - ...
- 有力仮説:
  - ...
- 同じ設計近傍か: yes/no
- 必要な場合の構造的に異なる分岐: ...
- 次に確認すること: ...
- 続行 / 停止の判断: ...

## Companion skills (推奨)

- `diversify-agent-search`: 利用可能で、候補アーカイブ、多様性軸、ケース別評価計画まで深掘りしたい場合

## 境界

### Always:

- 試行履歴を時系列で整理する
- 成功した確認と失敗した確認を分ける
- 仮説は最大 3 つに絞る
- 同じ設計近傍の試行を繰り返していないか確認する
- 次の一手は 1 つだけ提案する
- 関連 Skill が未導入でも、この Skill 単体で使えるようにする

### Ask first:

- 実装継続より停止が妥当な場合
- 追加変更が大きく、さらに失敗コストが増える場合

### Never:

- 同じ仮説のまま 3 回目の同系統修正に進まない
- 状況整理なしで変更だけを積み増さない
- 停止判断や構造的に異なる次の分岐を出す前に、別 Skill を必須にしない
