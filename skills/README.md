...
- kebab-case を使う
- 役割が伝わる短い名前にする
- まずは汎用名を優先し、リポジトリ固有やエージェント固有であることが重要な場合だけ、その前提が伝わる名前にする
- 利用できるツール、操作体系、入出力形式、実行手順が特定のエージェントに強く依存する場合は、必要に応じて agent 名の短い接頭辞を使ってよい
...
- 特定のエージェント固有前提を `name` に残すべきか、`description` や本文で表現すれば十分か確認したか
