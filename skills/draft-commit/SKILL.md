...
```text
<commit 1>
refactor: draft-commit の type 判定ルールを整理
files: skills/draft-commit/SKILL.md
git add skills/draft-commit/SKILL.md
git commit -m "refactor: draft-commit の type 判定ルールを整理"
```

```text
<commit 1>
refactor: skill rename の参照更新を整理
files: skills/design-skill/SKILL.md, skills/triage-agent-usage/SKILL.md
git add skills/design-skill skills/author-skill skills/triage-agent-usage skills/agent-budget-triage
git commit -m "refactor: skill rename の参照更新を整理"
```

## タイプ選定ガイド (例)

- 対象リポジトリに明示された規約がある場合は、その規約をこの一覧より優先する
- `feat`: 新機能
- `fix`: バグ修正
- `docs`: ドキュメント更新
