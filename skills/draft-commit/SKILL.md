...
11. 各コミット案について、`git add ...` と `git commit -m "..."` をそのまま実行できる形で並べる。
12. 提案は最大 3 案までに絞る。
13. タイプ選定の詳細が必要なら `references/commit-types.md` を参照する。
14. 差分が複数関心事に分かれ、かつユーザーが追加確認を求めた場合だけ、別 agent の利用を検討する。
