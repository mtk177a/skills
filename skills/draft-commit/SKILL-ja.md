...
- 差分をもとに、Conventional Commits 形式でコミット案を作る。
...
- 対象リポジトリに言語規約がない場合に使用する、ユーザーの使用言語
- `git status -sb`
- `git diff --stat`
...
1. まず現在作業中の対象リポジトリに明示されたコミット規約を確認し、コミット type、scope、summary の言語にローカル規約があればそれを優先する。
...
9. `summary` が短く具体的か確認する。対象リポジトリに指定言語があれば従い、なければユーザーの使用言語を使う。どちらからも判断できない場合は、提案前に確認する。
...
- summary の言語は、対象リポジトリの規約、ユーザーの使用言語の順で決める
- 関心事が複数なら分割して提案する
...
- 差分が取得できない場合
- diff から主目的が特定できない場合
- 対象リポジトリの規約とユーザーの使用言語のどちらからも summary の言語を判断できない場合
