...
5. `review` 段階で、正しさ、安全性、回帰、テスト不足の観点を確認する。
6. `review` で Must-fix が出た場合は、指摘と根拠を整理する。仕様変更や承認事項、高リスク変更に触れる場合は chain を止めて承認を取り、それ以外は `build` 段階へ戻して修正後に再確認する。
7. `review` の Must-fix が解消したら、Should-fix と Nice-to-have は未解決事項として整理し、`validate` 段階で確認済み、未確認、残留リスクを整理する。
...
- チェーン対象: ...
- 現在の段階: `intake` | `design` | `build` | `review` | `validate` | Done
- 直前の受け渡し要約:
  - 直前の段階: ...
  - 対象範囲 / 対象ファイル: ...
  - 実施したコマンド / テストと結果: ...
...
- 段階は `intake` -> `design` -> `build` -> `review` -> `validate` の順で進める
- `review` の Must-fix は、承認不要な範囲に限って `build` 段階へ戻す
- `review` の Should-fix / Nice-to-have は未解決事項として扱い、自動で再実装しない
...
- `build` より前に実装を始める
- `review` の重大指摘を無視して `validate` へ進む
