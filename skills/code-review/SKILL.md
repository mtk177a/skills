---
name: code-review
description: main agent が自分でコードレビューを行い、差分の指摘を整理したいときに使う。
---
...
- custom agent chain を起動せず、main agent 自身が local にレビュー結果を整理する入口として使う。
...
6. 必要なら `reviewer` を使ってレビュー結果を安定化させる。
7. 指摘の採否や対応方針が必要な場合は `triage-review` に渡す。
