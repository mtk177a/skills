...
このリポジトリ上でエージェントが従う運用規約は [AGENTS.md](AGENTS.md) を正本とします。
...
リポジトリルートには `apm.yml` を置き、このリポジトリ内 Skill の一部をまとめて導入できる curated package としても扱えるようにします。
...
ルートの `apm.yml` は全 Skill の自動収録を意味せず、収録対象は curated subset として管理します。
実際の導入や配布の管理は dotfiles 側または利用側プロジェクトの `apm.yml` / `apm.lock.yaml` で行い、このリポジトリ自体は Skill ソースの保管場所兼 curated APM package として扱います。
